...
public class Window {
	...
	private int width, height;                                //size of the game.window
	private boolean running = true;                            //false, if the game.window should be closed

	private List<Drawable> drawables;                        //list of drawables, that are used to draw the content of the screen
	private Queue<Drawable> toRemove;                        //list of drawables, that should be added to the game.window after the next draw
	...
	/**
	 * redraw loop
	 * while the game is running, this method updates the keyboard, the game.window and the drawables
	 */
	public void run() {
		while (running) {
	...
	/**
	 * Adds a drawable to this game.window
	 *
	...
	/**
	 * Removes a drawable from this game.window
	 *
	...
	/**
	 * Removes all used OpenGL instances and closes the game.window
	 */
	...
	private void initGLFW() {
		...
		if (window == MemoryUtil.NULL) ErrorUtil.printError("Creating game.window");
	...
	public void setFullscreen(boolean value) {
		/*if (value) {
			...
			GLFW.glfwSetWindowMonitor(game.window, GLFW.glfwGetPrimaryMonitor(), 0, 0, mode.width(), mode.height(), mode.refreshRate());
		} else {
			GLFW.glfwSetWindowSize(game.window, Constants.WINDOW_WIDTH, Constants.WINDOW_HEIGHT);
		}*/
	}
}
